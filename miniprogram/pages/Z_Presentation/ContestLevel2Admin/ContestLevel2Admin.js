...
Page({
  ...
  IsReady() {
    ...
    if (app.globalData.IsContestMatchReady == 0 || app.globalData.IsContestInfoReady == 0 || app.globalData.IsContestMemberReady == 0 || app.globalData.IsContestAuthorityNumReady == 0) {
      ...
      this.setData({
        ...
        ScoreItems: ContestInfo.Pre_ScoreItem,
        IdentifyCode:ContestInfo.Pre_IdentifyCode   //6位验证码
      })
      this.MergeTag();
  ...
  onLoad: function (options) {
    if (this.data.ContestId == "") {
      ...
      PF.GetAllMember(this.data.ContestId); //此处拉好数据给三级页面使用
      this.IsReady();
  ...
  AddScoreItem: function () {
    var newScoreItems = this.data.ScoreItems
    newScoreItems.push({
      ScoreContext: '',
      ScoreBound: '',
    })
    console.log(newScoreItems);
    this.setData({
      ScoreItems: newScoreItems
    })
  },

  DeleteScoreItem: function () {
    var newScoreItems = this.data.ScoreItems
    var len = newScoreItems.length;
    if (newScoreItems.length > 1) {
      newScoreItems = newScoreItems.slice(0, len - 1);
      console.log(newScoreItems);
      this.setData({
        ScoreItems: newScoreItems
      })
  ...
  BindScoreContextChange: function (e) {
    var id = e.currentTarget.dataset.id;
    var value = e.detail.value;
    var newScoreItems = this.data.ScoreItems;
    newScoreItems[id].ScoreContext = value;
    this.setData({
      ScoreItems: newScoreItems
    })
  },

  BindScoreBoundChange: function (e) {
    var id = e.currentTarget.dataset.id;
    var value = e.detail.value;
    var newScoreItems = this.data.ScoreItems;
    newScoreItems[id].ScoreBound = value;
    this.setData({
      ScoreItems: newScoreItems
    })
  },

  UpdateScoreItems: function () {
    var ScoreItems = this.data.ScoreItems;
    var len = ScoreItems.length;
    for (var i = 0; i < len; i++) {
      console.log(i, " Checked");
      if (ScoreItems[i].ScoreContext == '' || ScoreItems[i].ScoreBound == '') {
        wx.showToast({
          title: '有留空项目',
          icon: 'none',
          duration: 500
    ...
    console.log("Success", this.data.ScoreItems);
    PF.UpdateScoreItems(this.data.ContestRecordId, this.data.ScoreItems);
  },
  ...
  GetMatchInfo() {
    ...
    console.log("ContestMember", this.data.ContestMember);
    var len = 0;
    if (ContestMember != null) {
      len = ContestMember.length;
    }
    MatchInfo.forEach(function (item, index) {
      for (var i = 0; i < len; i++) {
        if (item.CurrentMemberId == ContestMember[i].MemberId) {
          item["CurrentMemberInfo"] = ContestMember[i];
          break;
        }
      }
    })
  ...
  ConfirmAdd(e) {
    var that = this;
    if (this.CheckMatchPlace() && this.CheckMatchName()) {
      this.setData({
        AddLevel2: 0
      })
      this.CreateMatchID();
      CF.Add_MatchInfo(this.data.ContestId, this.data.MatchId, this.data.MatchName, this.data.MatchPlace, " ", " ")
  ...
  DeleteMatch(e) {
    ...
    setTimeout(function () {
      that.setData({
        MatchInfo: app.TempData.ContestMatch.data,
      })
    }, 1000)
  },
  ...
  BindChangeMemberDescription: function (e) {
    console.log("e.detail.value", e.detail.value)
    this.setData({
      MemberDescription: e.detail.value
    })
  },
  ...
  CheckMemberDescription() {
    if (this.data.MemberDescription == "") {
      wx.showToast({
        title: "你还未填写描述",
        icon: 'none',
        duration: 1500
  ...
  ConfirmAddMember(e) {
    ...
    if (that.CheckMemberTag() && that.CheckMemberName() && that.CheckMemberDescription) {
      this.setData({
        AddMember: 0
      })
      ...
      CF.MainAddContestMember(this.data.ContestId, this.data.MemberName, this.data.MemberDescription, this.data.MemberTag, this.data.MemberId)
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          CF.UpdateContestInfo(that.data.ContestRecordId, that.data.ContestName, that.data.ContestPlace, that.data.StartDate, that.data.EndDate, that.data.ContestDescription, that.data.HoldOrganization, that.data.ContestTag)
        } else if (res.cancel) {
          console.log('用户点击取消')
        }
      }
  ...
  GotoContestLevel3Admin(e) {
    ...
    if (this.data.Authority == "Referee") {
      ...
      if (flag == 0) {
        ...
        wx.navigateTo({
          url: '../ContestLevel3Admin/ContestLevel3Admin?MatchId=' + e.currentTarget.dataset.id.MatchId +'&ContestId='+this.data.ContestId
        })
      }
    } else {
      wx.navigateTo({
  ...
  HandelMemberExcel() {
    ...
    ContestMember.forEach(function (item, index) {
      ...
      NewMemberDescription.push(item.MemberDescription);
    })
    ...
    ExcelData.forEach(function (item, index) {
      ...
      NewMemberDescription.push(item[2]);
    })
    console.log("NewMemberId,NewMemberName,NewMemberTag", NewMemberId, NewMemberName, NewMemberTag, NewMemberDescription);
    var NewMember = []
    NewMemberId.forEach(function (item, index) {
      NewMember.push({
        ...
        "NewMemberDescription": NewMemberDescription[index]
      });
    })
    this.setData({
    ...
    EF.UpdateContestMember(this.data.ContestMemberRecordId, NewMemberName, NewMemberDescription, NewMemberTag, NewMemberId)
