...
import PreFunction from "../../../js/PreFunction.js";
var app = getApp();
Page({
  ...
  data: {
    ...
    PreAllMember: [],
    PreAllScore: [],
    PreAllMemberLen: 0, //目前已经进入绑定参与评分的人数 
    TableHead: [],
    TableList: [],
    NeedFreshScore: 1,
    ScoreItem: []
  ...
  ExportExcel() {
    PF.exportFile(this.data.TableHead, this.data.TableList);
  },
  RefreshScore() {
    if (this.data.NeedFreshScore == 1) {
      PF.GetAllScore(this.data.MatchId);
      PF.GetAllMember(this.data.ContestId);
      this.IsGetReady();
    }
  },
  IsGetReady() {
    var that = this;
    console.log("app.globalData.IsPreGetAllScoreReady", app.globalData.IsPreGetAllScoreReady)
    if (app.globalData.IsPreGetAllScoreReady == 0 || app.globalData.IsPreGetAllMemberReady == 0) {
      setTimeout(function () {
        that.IsGetReady()
      }, 1000)
    } else {
      //数据已经存在如下两个数组中，进行分割即可
      app.globalData.IsPreGetAllScoreReady = 0;
      app.globalData.IsPreGetAllMemberReady = 0;
      this.setData({
        PreAllMember: app.TempData.PreAllMember,
        PreAllMemberLen: app.TempData.PreAllMember.length,
        PreAllScore: app.TempData.PreAllScore
      })
      console.log("PreAllMember,PreAllScore", this.data.PreAllMember, this.data.PreAllScore)
      this.SplitScore();
      setTimeout(function () {
        that.RefreshScore();
      }, 120000)
  ...
  SplitScore() {
    var that = this
    var NewMatchMember = this.data.MatchMember;
    var PreAllMember = this.data.PreAllMember;
    var PreAllScore = this.data.PreAllScore;
    console.log("NewMatchMember", NewMatchMember);
    console.log("PreAllMember,PreAllScore", PreAllMember, PreAllScore);
    NewMatchMember.forEach(function (item, index) {
      item.ScoreNum = 0; //该个成员有几个人对他打了分
      item.AllScore = 0;
      item.AverageScore = 0;
      item.Score = [];
    })
    PreAllScore.forEach(function (item, index) {
      //PreAllScore中能看到这一堆分数是谁打的了
      PreAllMember.forEach(function (item2, index2) {
        if (item.UserId == item2.UserId) {
          item.Name = item2.Name;
          item.StuId = item2.StuId;
        }
      })
    })
    console.log("NewMatchMember2", NewMatchMember);
    console.log("PreAllMember2,PreAllScore2", PreAllMember, PreAllScore);
    NewMatchMember.forEach(function (item, index) {
      PreAllScore.forEach(function (item2, index2) {
        item2.Score.forEach(function (item3, index3) {
          if (item3.MemberId == item.Info.MemberId) {
            var Temp = {
              "StuId": item2.StuId,
              "Name": item2.Name,
              "TotalScore": item3.TotalScore,
              "ItemScore": item3.ItemScore
            }
            item.Score.push(Temp);
            item.ScoreNum++;
            item.AllScore += parseInt(item3.TotalScore)
          }
        })
      })
      item.AverageScore = DC.toDecimal(item.AllScore / item.ScoreNum);
    })
    console.log("NewMatchMember3", NewMatchMember);
    var TableList = [];
    var TableHead = ["学号", "姓名"]
    var Name = ""
    NewMatchMember.forEach(function (item, index2) {
      var MemberName = item.Info.MemberName;
      var ScoreItem = that.data.ScoreItem
      ScoreItem.forEach(function (item, index) {
        Name = MemberName + '-' + item.ScoreContext + '(' + item.ScoreBound + '分)'
        TableHead.push(Name);
      })
      TableHead.push(MemberName + '-总分');
    })
    console.log("TableHead", TableHead)
    PreAllMember.forEach(function (item, index) {
      if (item.Name.substr(0,2) != '观众') {
        var Temp = {
          "学号": item.StuId,
          "姓名": item.Name,
        }
        NewMatchMember.forEach(function (item2, index2) {
          var flag = 0
          item2.Score.forEach(function (item3, index3) {

            if (item3.StuId == item.StuId && item3.Name == item.Name) {
              var MemberName = item2.Info.MemberName;
              var ScoreItem = that.data.ScoreItem
              ScoreItem.forEach(function (item4, index4) {
                Name = MemberName + '-' + item4.ScoreContext + '(' + item4.ScoreBound + '分)'
                Temp[Name] = item3.ItemScore[index4]
              })
              Temp[MemberName + '-总分'] = item3.TotalScore
              flag = 1;
            }
          })
          if (flag == 0) {
  ...
  IsReady() {
    ...
    if (app.globalData.IsChatRoomReady == 0 || app.globalData.IsMatchReady == 0 || app.globalData.IsMatchSendInfoReady == 0) {
      ...
      that.setData({
        ...
        ScoreItem: app.TempData.ContestInfo.Pre_ScoreItem
      })
      var CurrentStep = 0;
  ...
  SendNewInformation(e) {
    ...
    var New = {
      ...
      'Information': e.detail.value.NewInformation,
      'SendTime': this.data.Date.substr(5, 15) + ' ' + this.data.Hour_Minute
    };
    var NewMatchInfo = [];
